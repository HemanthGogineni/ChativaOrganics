...
@Component({
  ...
  animations: [
    // Stagger the product cards when the list first enters and when it changes
    trigger('list', [
      transition(':enter, * => *', [
        query(
          '.product-card',
...
export class ProductComponent implements AfterViewInit {
  ...
  constructor(private cartService: CartService, private router: Router, private route: ActivatedRoute, private cdr: ChangeDetectorRef, private ngZone: NgZone) { }
  ...
  sortBy: 'popularity' | 'priceAsc' | 'priceDesc' | 'rating' = 'popularity';
  ...
  ngOnInit() {
    ...
    // Listen for filter/search changes from query params
    this.route.queryParams.subscribe(params => {
      this.selectedCategory = params['category'] || '';
      this.searchText = params['search'] || '';

      // brief skeleton to telegraph grid update
      this.showGridSkeleton = true;
      this.scheduleSkeletonOff(260);
    });
  }

  ngAfterViewInit() {
    // Extra safety to ensure skeleton hides even if the initial timer misses due to hydration timing
    this.scheduleSkeletonOff(0);
  }

  private scheduleSkeletonOff(delay = 260) {
    const run = () => this.ngZone.run(() => this.endSkeleton());
    setTimeout(run, delay);
    // Failsafe in case initial timer is missed by hydration/zone timing
    setTimeout(run, 1200);
    // Microtask fallback to kick CD even sooner if needed
    Promise.resolve().then(run);
  }

  private endSkeleton() {
    if (this.showGridSkeleton) {
      this.showGridSkeleton = false;
  ...
  // Navigate to product detail immediately (no skeleton flip on list)
  goTo(id: string, event?: Event) {
    // If invoked from keyboard/outer container we don't want to stop navigation
    if (event) {
      // Prevent clicks from bubbling to other handlers that could delay navigation
  ...
  // Ensure add-to-cart button does not trigger card navigation
  onAddToCartClick(event: Event, product: any) {
    event.stopPropagation();
    event.preventDefault();
  ...
  trackByProductId(_: number, p: any) {
    return p.id;
  }
  ...
  applyCategory(category: string) {
    // show grid skeleton immediately for perceived responsiveness
    this.showGridSkeleton = true;
  ...
  // Prefetch product image on hover to speed up Product Detail load
  prefetchImage(imageName: string) {
    if (!imageName) return;
    const img = new Image();
    img.src = `assets/img/product-imgs/${imageName}`;
  }
}
